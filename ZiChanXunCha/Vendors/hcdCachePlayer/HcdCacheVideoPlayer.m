...
//!!!SL_2016-09-12
@property (nonatomic, strong) UIView * navigationItem;        //播放器顶部的导航

@property (nonatomic, strong) UIButton * backBtn;              //导航的返回按钮;

@property (nonatomic, strong) UILabel * titleLabel;             //导航标题;
...
//!!!SL_2016-09-12 设置全屏状态下的NavigationBar

-(UIView *)navigationItem{
    if (!_navigationItem) {
        _navigationItem = [[UIView alloc]init];
        _navigationItem.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.5];
        _navigationItem.hidden = YES;
    }
...
-(UIButton *)backBtn{
    if (!_backBtn) {
        ...
        [_backBtn setImage:[UIImage imageNamed:HcdImageSrcName(@"icon_back_white")] forState:UIControlStateNormal];
        [_screenButton setImage:[UIImage imageNamed:HcdImageSrcName(@"icon_back_white")] forState:UIControlStateHighlighted];
    }
    return _backBtn;
}
...
//!!!SL-2016-09-12 设置navigationItem
-(void)setNavigationItemView{
    
    [_showView addSubview: self.navigationItem];
    [self.navigationItem addSubview:self.backBtn];
    
    [_navigationItem mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(0);
        make.top.mas_equalTo(0);
        make.right.mas_equalTo(0);
